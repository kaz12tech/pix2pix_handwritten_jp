...
def spectral_normed_weight(W, u=None, num_iters=1, update_collection=None, with_sigma=False):
	...
	def power_iteration(i, u_i, v_i):
		v_ip1 = _l2normalize(tf.matmul(u_i, tf.transpose(a=W_reshaped)))
	...
	if update_collection is None:
		...
		sigma = tf.matmul(tf.matmul(v_final, W_reshaped), tf.transpose(a=u_final))[0, 0]
...
def dropout(x, rate=0.5, training=True):
	return tf.compat.v1.layers.dropout(x, rate=0.5, training=training)
...
def conv2d(x, output_dim, kernel_size=4, strides=2, padding = "SAME", spectral_normed=False,
	...
	with tf.compat.v1.variable_scope(name) as scope:
		w = tf.compat.v1.get_variable("w", [kernel_size, kernel_size, x.get_shape()[-1], output_dim],
							initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev))
		biases = tf.compat.v1.get_variable("b", [output_dim], initializer=tf.compat.v1.constant_initializer(0.0))
		if spectral_normed:
			conv = tf.nn.conv2d(input=x, filters=spectral_normed_weight(w, update_collection=update_collection),
								strides=[1, strides, strides, 1], padding=padding)
		else:
			conv = tf.nn.conv2d(input=x, filters=w, strides=[1, strides, strides, 1], padding=padding)
...
"""def conv2d(x, num_outputs, kernel_size=4, strides=2, padding = "SAME", stddev = 0.02, name="conv_2d"):
	return tf.contrib.layers.conv2d(x, num_outputs, kernel_size, strides, padding,\
	  activation_fn=None, weights_initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev),biases_initializer=tf.constant_initializer(0.0))
"""	
def deconv2d(x, num_outputs, kernel_size=4, strides=2, padding = "SAME", stddev = 0.02, name="conv_2d"):
	...
	#  activation_fn=None, weights_initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev),biases_initializer=tf.compat.v1.constant_initializer(0.0))
	return tf.compat.v1.layers.conv2d_transpose(
		x, num_outputs, kernel_size, strides, padding,
		#activation_fn=None, 
...
def residule_block(x, dim, name="residule_block"):
	with tf.compat.v1.variable_scope(name) as scope:
		y = tf.pad(tensor=x, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
		y = conv2d(y, dim, 3, strides=1, padding='VALID', name='r_conv_1')
		y = tf.nn.relu(instance_norm(y,'r_bn_1'))

		y = tf.pad(tensor=y, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
